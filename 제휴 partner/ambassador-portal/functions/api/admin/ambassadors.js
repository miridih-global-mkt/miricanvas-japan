...
// POST /api/admin/ambassadors {name, description} — 등록 (전용 링크 자동 발급)
...
export async function onRequestGet({ request, env }) {
  ...
  const { results } = await env.DB.prepare(
    `SELECT a.id, a.name, a.token, a.description, a.active, a.created_at,
...
export async function onRequestPost({ request, env }) {
  ...
  ).bind(name, token, (body.description || '').trim() || null).run();
